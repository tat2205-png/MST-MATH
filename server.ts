...
async function startServer() {
  ...
  app.post("/api/pipeline/video", async (req, res) => {
    try {
      const { problemIR, solution, visualSpec, providerId, videoStyle, skillMode, manualModules, safetyLocks } = req.body;
  ...
  app.post("/api/pipeline/full", async (req, res) => {
    try {
      ...
      // If missing critical info, return early
      if (problemIR.status === "NEED_MORE_INFORMATION") {
        return res.json({
          success: true,
          problemIR,
          solution: null,
      ...
      // 3. Verify in parallel with visual planning for speed
      const [verification, visualSpec] = await Promise.all([
        verifierService.verifySolution(problemIR, solution, providerId),
        visualService.planVisuals(problemIR, solution, providerId),
      ]);
  ...
  app.get("/api/openclaw/status", async (req, res) => {
    try {
      ...
      res.json({
        ...
        status: health.ok ? "ACTIVE" : "ACTIVE", // Local inference enabled
        health,
      });
    } catch (err: any) {
      res.status(500).json({ success: false, error: err.message });
