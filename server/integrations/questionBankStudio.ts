...
function hasAuthoritativeSourceSolution(question: QuestionObject): boolean {
  return Boolean(question.solution?.length && question.id && question.source.document && question.source.sourceHash && question.source.blockIds.length && question.source.sourceLocations.length);
}
function sourceAnswer(question: QuestionObject): string {
  const explicit = blocksText(question.answer);
  if (explicit) return explicit;
  const match = blocksText(question.solution).match(/(?:^|[.!?;]\s*)(?:đáp\s*án(?:\s+đúng)?|chọn)\s*[:\-]?\s*([ABCD])(?=$|[\s.)!,;:])/iu);
  return match?.[1]?.toUpperCase() ?? "";
}
function sourceAnswerBlocks(question: QuestionObject): ContentBlock[] | undefined {
  const answer = sourceAnswer(question);
  return answer ? [{ type: "text", value: answer }] : undefined;
}
...
def fit_block(block, max_width, max_height):
    if block.width <= 0 or block.height <= 0:
        return block
    block.scale(min(1, max_width / block.width, max_height / block.height))
    return block
...
def prose_block(value, font_size, max_width, max_height, font=FONT_BODY):
    ...
    return fit_block(block, max_width, max_height)
