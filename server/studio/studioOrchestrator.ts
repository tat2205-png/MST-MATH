...
import { EngineRegistry, StudioEngineRegistry, studioEngineRegistry, type CapabilityStatus } from "./engineRegistry.js";
...
export interface StudioRequest { intent?: string; taskType?: "math" | "geometry" | "visual" | "image" | "render" | "unknown"; problemType?: string; geometryType?: string; source?: string; outputFormat?: "mp4" | "png" | "gif" | "json"; }
export interface StudioPlanStep { capability: string; engine: string; status: CapabilityStatus; rationale: string; }
export interface StudioExecutionPlan { featureEnabled: boolean; routeId: "MATH" | "MANIM" | "LUADRAW" | "IMAGE_ANIMATION" | "RENDER_ONLY" | "UNKNOWN"; selectedCapabilities: string[]; selectedEngines: string[]; steps: StudioPlanStep[]; rationale: string; outputFormat: "mp4" | "png" | "gif" | "json"; }

export class StudioOrchestrator {
  constructor(
    private readonly registry: EngineRegistry = new StudioEngineRegistry(),
  ...
  plan(request: StudioRequest): StudioExecutionPlan {
    ...
    if (!StudioEngineRegistry.featureFlagEnabled()) return { featureEnabled: false, routeId: "UNKNOWN", selectedCapabilities: [], selectedEngines: [], steps: [], rationale: "Studio Orchestrator is disabled by default. Enable STUDIO_ORCHESTRATOR_V1=true.", outputFormat };
    const intent = (request.intent ?? "").toLowerCase();
    const geometryType = (request.geometryType ?? "").toUpperCase();
    if (/(cube|polyhedron|net|tetra|prism|pyramid|octa)/.test(intent) || geometryType.includes("POLYHEDRON")) return this.buildCompatibilityPlan("LUADRAW", ["geometry.2d", "geometry.luadraw", "render.video"], "Validated geometry and LuaDraw route.", outputFormat);
    if (/(triangle|circle|line|shape|geometry|diagram)/.test(intent) || request.taskType === "geometry") return this.buildCompatibilityPlan("MANIM", ["geometry.2d", "visual.manim", "render.video"], "Validated geometry and Manim route.", outputFormat);
    if (/(equation|solve|algebra|system|inequality|function)/.test(intent) || request.taskType === "math") return this.buildCompatibilityPlan("MATH", ["math.solve", "math.analyze", "render.video"], "Deterministic math route.", outputFormat);
    if (/(image|animate|motion|depth|segmentation|blender|generative)/.test(intent) || request.taskType === "image") return this.buildCompatibilityPlan("IMAGE_ANIMATION", ["animation.image", "animation.motion", "animation.depth25d", "render.video"], "Image animation route with explicit optional-runtime status.", outputFormat);
    return this.buildCompatibilityPlan("UNKNOWN", ["math.solve"], "No rule matched; fail-closed route.", outputFormat);
  }

  private buildCompatibilityPlan(routeId: StudioExecutionPlan["routeId"], capabilities: string[], rationale: string, outputFormat: StudioExecutionPlan["outputFormat"]): StudioExecutionPlan {
    const legacy = this.registry instanceof StudioEngineRegistry ? this.registry : studioEngineRegistry;
    const steps = capabilities.map((capability) => { const record = legacy.getCapability(capability); return { capability, engine: record?.engine ?? "unknown-engine", status: record?.status ?? "UNAVAILABLE", rationale: record?.notes ?? "Capability unavailable." }; });
    return { featureEnabled: true, routeId, selectedCapabilities: capabilities, selectedEngines: [...new Set(steps.map((step) => step.engine))], steps, rationale, outputFormat };
  }
