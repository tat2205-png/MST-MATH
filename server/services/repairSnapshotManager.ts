...
export class RepairSnapshotManager {
  ...
  public generateFingerprints(params: {
    ...
    const { problemText = "", formulas = [], geometryFacts = [], graphSpec, sourceFiles } = params;
    const trustedFormulas = [...formulas];

    const mathContent = JSON.stringify({
      problemText,
      formulas: [...trustedFormulas].sort(),
    });
    ...
    const sourceText = Object.values(sourceFiles || {}).join("\n");
    const normalizeFormula = (formula: string) => formula.replace(/\s+/g, "");
    const formulaFingerprints = sourceFiles
      ? trustedFormulas.filter((formula) => sourceText.replace(/\s+/g, "").includes(normalizeFormula(formula)))
      : trustedFormulas;
