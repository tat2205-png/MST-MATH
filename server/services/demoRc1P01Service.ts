...
export async function buildDemoRc1P01(fileName: string, bytes: Uint8Array, outputDirectory: string): Promise<DemoRc1P01Result> {
  ...
  const mathQA = runProductionMathQA({ raw: ingested.document, normalized });
  const lesson = buildP01LessonIR(ingested.document);
  if (ingested.status !== "PASS" || normalized.review.length || mathQA.status !== "PASS") return { status: "REVIEW_REQUIRED", sourceHash: ingested.document.sourceHash, diagnostics: [...ingested.diagnostics, ...normalized.review, ...mathQA.gates.flatMap((gate) => gate.issues)], mathQA, lesson };
  ...
  return { status: "PASS", sourceHash: ingested.document.sourceHash, diagnostics: [], qa: bundle.qa, mathQA, lesson, artifacts };
}
