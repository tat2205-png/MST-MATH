...
export class VisualFrameQAService {
  ...
  async runJobFrameQA(params: {
      ...
      try {
        ...
        if (artifactsRes.ok) {
          ...
          for (const art of artifacts) {
            ...
            const artUrl = art.pathOrUrl || art.url || `${bridgeUrl}/api/jobs/${encodeURIComponent(jobId)}/artifacts/${art.id || art.name}`;

            if ((name.includes("START") || art.type === "start_frame") && !startFetchOk) {
              const b64 = await this.downloadImageBase64(artUrl, headers);
              if (b64) {
                frameSources.push({ frameName: "START", name: art.name || "START.png", base64: b64, url: artUrl });
                startFetchOk = true;
              }
            } else if ((name.includes("KEY") || art.type === "key_frame") && !keyFetchOk) {
              const b64 = await this.downloadImageBase64(artUrl, headers);
              if (b64) {
                frameSources.push({ frameName: "KEY", name: art.name || "KEY.png", base64: b64, url: artUrl });
                keyFetchOk = true;
              }
            } else if ((name.includes("END") || art.type === "end_frame") && !endFetchOk) {
              const b64 = await this.downloadImageBase64(artUrl, headers);
              if (b64) {
                frameSources.push({ frameName: "END", name: art.name || "END.png", base64: b64, url: artUrl });
                endFetchOk = true;
              }
            }
          }
    ...
    // 3. Multimodal Analysis for each frame
    let geminiVisionOk = true;
    const startResult = await this.evaluateSingleFrame("START", frameSources.find((f) => f.frameName === "START"), {
      problemIR,
      solution,
      visualSpec,
      videoSpec,
    });
    const keyResult = await this.evaluateSingleFrame("KEY", frameSources.find((f) => f.frameName === "KEY"), {
      problemIR,
      solution,
      visualSpec,
      videoSpec,
    });
    const endResult = await this.evaluateSingleFrame("END", frameSources.find((f) => f.frameName === "END"), {
      problemIR,
      solution,
      visualSpec,
      videoSpec,
...
Hãy trả về JSON theo schema:
{
  ...
  private normalizeIssue(raw: any): FrameQAIssue {
    ...
    let repairClass: "SAFE_AUTO_REPAIR" | "REVIEW_REQUIRED" = "REVIEW_REQUIRED";
    if (
      category === "MATH_ERROR" ||
   ...
   */
  private async downloadImageBase64(
    url: string,
