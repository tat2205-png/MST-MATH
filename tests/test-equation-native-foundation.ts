...
function createEquationNativeFixture(): Uint8Array {
  ...
  writeDirectoryEntry(directory, 128, "Equation Native", 2, 0, 64);

  const rootMiniStream = bytes.subarray(1536, 2048);
  rootMiniStream[0] = 0x1c;
  rootMiniStream[1] = 0x00;
  rootMiniStream[28] = 5;
  rootMiniStream.set(new TextEncoder().encode("DSMT5"), 32);
...
const bytes = createEquationNativeFixture();
const profile = findEquationNative(bytes);

assert(profile);
assert.equal(profile.stream, "Equation Native");
assert.equal(profile.payloadOffset, 28);
assert.equal(profile.version, 5);
assert.equal(profile.product, "DSMT5");
assert(equationNativeStream(bytes)?.bytes.length);
