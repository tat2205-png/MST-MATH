...
const base = make({ sourceRaw: "x", latex: "  x  ", parseStatus: "PARSED" }); const normalized = normalizeDocument(base); const proof = { status: "VERIFIED" as const, verifier: "existing-deterministic-math-verifier", evidence: "fixture-proof:x=2" }; const good: MathQAInput = { raw: base, normalized, questions: [{ id: "q1", expressions: [{ raw: "x", normalized: "x", parseStatus: "PARSED", sourceLocation: "fixture:p:0:math" }], rawDomainConditions: ["x > 0"], normalizedDomainConditions: ["x > 0"], expectedAnswer: "2", providedAnswer: "2", answerVerification: proof, provenance: [{ originalValue: "x", normalizedValue: "x", rule: "TRIM", sourceLocation: "fixture:p:0:math" }] }] };
assert.equal(runMathQA(good).status, "PASS");
assert.equal(runMathQA(good).gates.length, 8);
assert.equal(runMathQA({ ...good, questions: [{ ...good.questions![0], answerVerification: undefined }] }).status, "NEEDS_HUMAN_REVIEW");
assert.equal(runMathQA({ ...good, questions: [{ ...good.questions![0], expectedAnswer: "2", answerVerification: undefined }] }).status, "NEEDS_HUMAN_REVIEW");
assert.notEqual(runMathQA({ ...good, questions: [{ ...good.questions![0], answerVerification: { status: "UNSUPPORTED", verifier: "existing-deterministic-math-verifier", evidence: "" } }] }).status, "PASS");
assert.equal(runMathQA({ ...good, questions: [] }).status, "BLOCKED");
assert.equal(runMathQA({ ...good, questions: [{ ...good.questions![0], expressions: [] }] }).status, "BLOCKED");
