...
import { PIMATH_DNA, resolveOutputProfile } from "../src/config/naMathBrandRoot.ts";
...
for (const p of [thpt, dgnl, sat]) { assert.equal(p.inheritedFrom, PIMATH_DNA.standardId); assert.equal(p.answerPagePolicy.required, true); assert.equal(p.answerPagePolicy.answersOnly, true); assert.equal(p.answerPagePolicy.solutions, false); }
...
console.log("PIDNA_05_EXAM_PROFILES_QA=PASS");
