...
import { MST_MATH_DNA, resolveOutputProfile } from "../src/config/mstMathBrandRoot.ts";
...
const expectedProfileIds = [
  ...
  "P06_EXAM_SAT",
  "P07_VIDEO",
  "MST_MATH_VIDEO_VISUAL_CANONICAL_V2.0",
  "P08_GEOGEBRA",
  "P09_FOLD",
  "P10_GAME",
...
assert.equal(actualProfileIds.includes("MST_MATH_VIDEO_VISUAL_CANONICAL_V2.0"), true);
assert.equal(registry.profiles.find((profile) => profile.profileId === "MST_MATH_VIDEO_VISUAL_CANONICAL_V2.0")?.canonical, true);
assert.equal(registry.profiles.find((profile) => profile.profileId === "MST_MATH_VIDEO_VISUAL_CANONICAL_V2.0")?.approved, true);
...
for (const p of [thpt, dgnl, sat]) {
  assert.equal(p.inheritedFrom, MST_MATH_DNA.standardId);
  assert.equal(p.answerPagePolicy.required, true);
  assert.equal(p.answerPagePolicy.answersOnly, true);
  assert.equal(p.answerPagePolicy.solutions, false);
}
...
assert.equal(thpt.examSpec.promptCount, 34);
assert.equal(thpt.examSpec.questionCount, 22);
assert.equal(thpt.examSpec.durationMinutes, 90);
assert.deepEqual(dgnl.examSpec.questionRange, [61, 90]);
assert.equal(dgnl.examSpec.keepQuestionTogether, true);
assert.equal(dgnl.examSpec.columns, 1);
...
assert.equal(sat.examSpec.modules.length, 2);
assert.equal(sat.examSpec.sprDedicatedResponseArea, true);
