...
test("TX-03 and TX-04 restore after callback failure", async () => { const f = fake(); const tx = new GeoGebraSessionTransaction(f.adapter, "artifacts/mst07/session-recovery/test.ggb"); await assert.rejects(() => tx.run(async () => { f.adapter.newConstruction(); throw new Error("SIMULATED_SERIALIZATION_FAILURE"); }), /SIMULATED_SERIALIZATION_FAILURE/); assert.equal((await f.adapter.readSessionState()).objectNames[0], "original"); });

test("TX-10 reports critical restore failure", async () => { const f = fake(); f.adapter.setBase64 = async () => {}; const tx = new GeoGebraSessionTransaction(f.adapter, "artifacts/mst07/session-recovery/test2.ggb"); await assert.rejects(() => tx.run(async () => { throw new Error("boom"); }), /CRITICAL_RESTORE_FAILURE/); });
