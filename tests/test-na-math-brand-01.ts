...
import { MST_MATH_DNA, PIMATH_DNA, NA_MATH_OUTPUT_PROFILES, resolveBrand, resolveOutputProfile, resolveCanonicalReference, resolveConsumerProfile, resolveIcon, resolveComponent, resolvePdfLatexAuthority } from "../src/config/naMathBrandRoot.ts";
...
assert.equal(MST_MATH_DNA.canonical, true); assert.equal(MST_MATH_DNA.singleSourceOfTruth, true);
assert.equal(PIMATH_DNA, MST_MATH_DNA);
assert.equal(NA_MATH_OUTPUT_PROFILES.length, 17);
assert.ok(NA_MATH_OUTPUT_PROFILES.every((profile) => profile.parentBrandId === "MST-MATH-DNA-V1.0"));
...
assert.throws(() => resolveIcon("default"), /MST_MATH_DNA_AUTHORITATIVE_TOKEN_UNRESOLVED/);
