...
assert.ok(p01.canonicalReferences.includes("PIMATH-DNA-SEMANTIC-ICONS-V2.0"));
...
assert.deepEqual(resolveOutputProfile("P07_VIDEO").canonicalReferences, ["NA_MATH_CANONICAL_LAYOUT_V1_3", "NA_MATH_VIDEO_VISUAL_LANGUAGE_V1_0", "NA_MATH_VIDEO_GOLDEN_START_MID_END_V1", "PIMATH-DNA-SEMANTIC-ICONS-V2.0"]);
...
assert.throws(() => resolveIcon("default"), /NON_CANONICAL_ICON_ROLE/);
const coreIconRoles = ["LEARNING_OBJECTIVE", "CONCEPT", "DEFINITION", "FORMULA", "EXAMPLE", "ACTIVITY", "QUESTION", "EXERCISE", "REASONING", "TIP", "NOTE", "IMPORTANT", "WARNING", "ANSWER", "SOLUTION", "GEOMETRY", "GRAPH", "TABLE"] as const;
assert.equal(coreIconRoles.length, 18);
const masterResources = new Set<string>();
for (const role of coreIconRoles) {
  const icon = resolveIcon(role);
  assert.equal(icon.authority, "PIMATH-DNA-SEMANTIC-ICONS-V2.0");
  assert.equal(icon.semanticRole, role);
  assert.match(icon.resource, /^assets\/pimath-icons\/canonical\/v2\/.*\.svg$/);
  masterResources.add(icon.resource);
}
assert.equal(masterResources.size, 18);
assert.equal(resolveIcon("REASONING").resource, "assets/pimath-icons/canonical/v2/reasoning.svg");
for (const retiredRole of ["QUESTION_SOURCE", "SOLUTION_REASONING", "GEOMETRY_FIGURE", "RESULT_SUCCESS", "APPLICATION", "EXPLORE", "REFLECTION", "REAL_WORLD_MATH", "THINKING", "PROOF", "LOGIC", "INFERENCE"] as const) {
  assert.throws(() => resolveIcon(retiredRole), new RegExp(`NON_CANONICAL_ICON_ROLE:${retiredRole}`));
}
const canonicalSvgFiles = readdirSync("assets/pimath-icons/canonical/v2").filter((name) => name.endsWith(".svg"));
assert.equal(canonicalSvgFiles.length, 18);
const rootLegacySvgFiles = readdirSync("assets/pimath-icons").filter((name) => name.endsWith(".svg"));
assert.deepEqual(rootLegacySvgFiles, []);
...
assert.equal(resolveComponentIcon("proof", "DOCX").role, "REASONING");
