...
const imageBeforeBoundary = importDocxDetailed(makeDocx(p(`<w:r><w:drawing><a:blip r:embed="rId5"/></w:drawing></w:r>${t("Bài 4. Tính giá trị.")}`), true), "image-before-boundary.docx"); assert.equal(imageBeforeBoundary.candidates.length, 1); assert.equal(imageBeforeBoundary.candidates[0].source.questionNumber, "4"); assert.equal(imageBeforeBoundary.candidates[0].assetIds.length, 1);
const imageOnlyFallback = importDocxDetailed(makeDocx(p(`<w:r><w:t></w:t><w:drawing><a:blip r:embed="rId5"/></w:drawing></w:r>`), true), "image-only.docx"); assert.equal(imageOnlyFallback.candidates[0].status, "QUARANTINED"); assert.equal(imageOnlyFallback.candidates[0].assetIds.length, 1); assert.equal(imageOnlyFallback.diagnostics.assetsMapped, 1); assert.ok(imageOnlyFallback.candidates[0].notes.includes("UNRELIABLE_IMAGE_ONLY_EXTRACTION")); assert.ok(!imageOnlyFallback.candidates[0].content.some((b) => b.type === "text" && !b.value));
const imageDominatedFallback = importDocxDetailed(makeDocx(`${p(`${t("header")}<w:r><w:drawing><a:blip r:embed="rId5"/></w:drawing></w:r>`)}${p(`<w:r><w:drawing><a:blip r:embed="rId5"/></w:drawing></w:r>`)}${p(`<w:r><w:drawing><a:blip r:embed="rId5"/></w:drawing></w:r>`)}`, true), "image-dominated.docx"); assert.equal(imageDominatedFallback.candidates[0].status, "DRAFT");
const unmappedTable = importDocxDetailed(makeDocx(`<w:tbl><w:tr><w:tc>${p(t("metadata"))}</w:tc></w:tr></w:tbl>${p(t("Câu 1. Bài toán"))}`), "unmapped-table.docx"); assert.equal(unmappedTable.diagnostics.tables, 1); assert.equal(unmappedTable.diagnostics.tablesMapped, 0); assert.equal(unmappedTable.diagnostics.unmappedTables, 1); assert.ok(unmappedTable.document.warnings.includes("UNMAPPED_TABLES:1"));
...
const whitespaceIr = parseDocxToDocumentIR(makeDocx(p(`<w:r><w:tab/></w:r>${t("Câu 1. Nội dung")}<w:r><w:br/></w:r>`)), "whitespace.docx").document; const whitespaceParagraph = whitespaceIr.blocks[0]; assert.equal(whitespaceParagraph.type, "paragraph"); if (whitespaceParagraph.type === "paragraph") assert.ok(whitespaceParagraph.content.every((block) => block.type !== "text" || block.value.trim().length > 0));
