...
import { calibratedDecision } from "../src/modules/question-bank/micro-gold-calibration.js";
...
for(const file of files){const bytes=new Uint8Array(readFileSync(`${root}/${file}`)); const doc=ingestDocx({name:file,bytes}).document; const qs=doc?segmentQuestions(doc):[]; for(let i=0;i<qs.length;i++){const id=`${file}::candidate-${i+1}`; const state=id===falseId?"REJECTED_FALSE":reviewIds.has(id)?"REVIEW":"CONFIRMED"; base.push({candidateId:id,state,decisionSource:"CERTIFIED_SNAPSHOT"}); if(reviewIds.has(id)){const human=exact.get(id) as string|undefined; const d=human??calibratedDecision(qs[i]!); if(d && d!=="UNRESOLVED") decisions.set(id,{candidateId:id,state:toState(d),decisionSource:human?"HUMAN_APPROVED_MICRO_GOLD":"CALIBRATION_RULE"});}}
 docs.push({file,hash:createHash("sha256").update(bytes).digest("hex"),candidateCount:qs.length});}
const composed=composeBoundaryState(base,reviewIds,decisions); if(!Object.values(composed.invariants).every(Boolean)) throw new Error("STATE_COMPOSITION_INVARIANT_FAILED");
mkdirSync(out,{recursive:true}); const byFile=docs.map(d=>({...d,...Object.fromEntries(["CONFIRMED","REVIEW","REJECTED_FALSE","MERGE_PREVIOUS","MERGE_NEXT"].map(s=>[s.toLowerCase(),composed.candidates.filter(x=>x.candidateId.startsWith(`${d.file}::`)&&x.state===s).length]))}));
