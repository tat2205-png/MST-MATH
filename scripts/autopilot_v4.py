...
def bootstrap(project_root: Path, automation_root: Path, schema_path: Path,
              codex: Path | None) -> int:
    print_section("MATH AI VIDEO STUDIO — AUTOPILOT V4 BOOTSTRAP")
    report: dict[str, Any] = {
        "timestamp": now_iso(),
    ...
    for key, command in (
        ...
        try:
            code, out = run_cmd(command, project_root)
            if code == 0 and out.strip():
                report[key] = out.splitlines()[0].strip()
        except Exception:
            pass
...
"""
    ...
    print("AUTOPILOT_V4_READY: " + ("PASS" if report["overall"] == "PASS" else "FAIL"))
...
def get_plan_with_retries(
    *,
    ...
    for attempt in range(max_retries + 1):
        ...
        print_section(
            f"AUTOPILOT V4 — {'REPLAN' if attempt else 'FILE PLAN'} {attempt}/{max_retries}"
        )
...
def main() -> int:
    ...
    try:
        ...
        print_section("AUTOPILOT V4 — TRANSACTION APPLY")
        try:
        ...
        print_section("AUTOPILOT V4 — FULL DEV LOOP")
...
"""
            ...
            print_section(f"AUTOPILOT V4 — FULL DEV LOOP RETEST {repair}/{args.max_repair_attempts}")
