...
const certification = { ...common, execution: { answerSolutionValidatorDocumentsRun: files.length, questionTypeValidatorQuestionCount: typeRows.length, packageValidatorPackageCount: packageChecks.length, sourceObjectValidatorDocumentsRun: files.length }, qa: { boundaryConsumption: questions.length === 670, identityBridge: new Set(questions.map(q => `${q.sourceDocumentId}::${q.sourceObjectIds.join(",")}`)).size === 670, answerSolution: answer.valid && solution.valid, questionTypeAccounting: Object.values(types).reduce((a: number, b: any) => a + b, 0) === 670, questionTypeSemantic: typeRows.every(x => x.qaStatus === "PASS"), package: packageValid, sourceObjectAccounting: sourceValid, sourceCorpusImmutability: hashMatch === 11 }, automatedWordBetaQA: questions.length === 670 && packages.length === 670 && answer.valid && solution.valid && packageValid && sourceValid && hashMatch === 11 ? "PASS" : "BLOCKED_VALIDATION" };
