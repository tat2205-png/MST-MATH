...
export class LocalBridgeClient {
  ...
  public async getRenderArtifacts(jobId: string): Promise<RenderArtifact[]> {
    ...
    try {
      ...
      const normalized = this.normalizeArtifacts(rawArtifacts);
      return Promise.all(normalized.map((artifact) => this.hydrateImageArtifact(artifact)));
    } catch (err: any) {
      if (err.message === LOCAL_BRIDGE_ERRORS.TOKEN_REQUIRED_OR_INVALID) {
        throw err;
      }
  ...
  private async hydrateImageArtifact(artifact: RenderArtifact): Promise<RenderArtifact> {
    if (!artifact.type.endsWith("_frame") || !artifact.pathOrUrl) return artifact;
    try {
      const artifactUrl = this.resolveArtifactUrl(artifact.pathOrUrl);
      const response = await fetch(artifactUrl, { method: "GET", headers: this.getApiHeaders() });
      if (!response.ok) return artifact;
      const mimeType = response.headers.get("content-type")?.split(";")[0] || artifact.mimeType || "image/png";
      if (!mimeType.startsWith("image/")) return artifact;
      const bytes = new Uint8Array(await response.arrayBuffer());
      if (bytes.length === 0) return artifact;
      let binary = "";
      ...
      return {
        ...
        previewDataUrl: `data:${mimeType};base64,${btoa(binary)}`,
      };
    } catch {
      return artifact;
    }
  ...
  private resolveArtifactUrl(pathOrUrl: string): string {
    const base = new URL(this.bridgeUrl);
    const resolved = new URL(pathOrUrl, base);
    if (resolved.protocol !== "http:" && resolved.protocol !== "https:") {
      throw new Error("Unsupported artifact URL protocol");
    }
    if (resolved.origin !== base.origin) {
      throw new Error("Artifact URL escaped the configured Local Bridge origin");
    }
