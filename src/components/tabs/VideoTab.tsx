...
import { MasterCanvas, MasterCanvasQa } from "../../types/cinematicCanvas.js";
...
export const VideoTab: React.FC<VideoTabProps> = ({
  ...
  const [masterCanvas, setMasterCanvas] = useState<MasterCanvas | null>(null);
  const [masterCanvasQa, setMasterCanvasQa] = useState<MasterCanvasQa | null>(null);
  const [isBuildingCanvas, setIsBuildingCanvas] = useState(false);
  const [canvasError, setCanvasError] = useState<string | null>(null);
  const [showCameraPlan, setShowCameraPlan] = useState(false);

  const handleBuildMasterCanvas = async () => {
    setIsBuildingCanvas(true);
    setCanvasError(null);
    try {
      const response = await fetch("/api/cinematic/master-canvas", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ videoStyle, problemIR, solution, visualSpec, videoSpec }),
      });
      const data = await response.json();
      if (!response.ok || !data.success) throw new Error(data.error || "Không thể tạo Master Canvas.");
      setMasterCanvas(data.masterCanvas || null);
      setMasterCanvasQa(data.qa || null);
    } catch (err: any) {
      setCanvasError(err.message || "Không thể tạo Master Canvas.");
    } finally {
      ...
      {videoStyle === "cinematic_infographic" && (
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm space-y-3">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <h2 className="text-xs font-bold uppercase tracking-wider text-slate-800 flex items-center gap-1.5">
                <Layers className="w-4 h-4 text-blue-600" /> Cinematic Master Canvas (8A)
              </h2>
              <p className="text-[11px] text-slate-500 mt-1">Planner data only; chưa render.</p>
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={handleBuildMasterCanvas}
                disabled={isBuildingCanvas}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white text-xs font-semibold"
              >
                <RefreshCw className={`w-3.5 h-3.5 ${isBuildingCanvas ? "animate-spin" : ""}`} />
                Build Master Canvas
              </button>
              <button
                onClick={() => setShowCameraPlan((current) => !current)}
                disabled={!masterCanvas}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-50 disabled:opacity-50 text-xs font-semibold"
              >
                <Camera className="w-3.5 h-3.5" /> Preview Camera Plan
              </button>
            </div>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
            <div className="p-2 rounded-lg bg-slate-50 border border-slate-200"><span className="block text-[10px] text-slate-500">Canvas Status</span><strong>{masterCanvasQa?.MASTER_CANVAS_QA || "NOT_BUILT"}</strong></div>
            <div className="p-2 rounded-lg bg-slate-50 border border-slate-200"><span className="block text-[10px] text-slate-500">Regions</span><strong>{masterCanvas?.regions.length || 0}</strong></div>
            <div className="p-2 rounded-lg bg-slate-50 border border-slate-200"><span className="block text-[10px] text-slate-500">Camera Shots</span><strong>{masterCanvas?.cameraShots.length || 0}</strong></div>
            <div className="p-2 rounded-lg bg-slate-50 border border-slate-200"><span className="block text-[10px] text-slate-500">Narration Cues</span><strong>{masterCanvas?.regions.reduce((count, region) => count + region.narrationCueIds.length, 0) || 0}</strong></div>
          </div>
          {canvasError && <p className="text-xs text-rose-700">{canvasError}</p>}
          {masterCanvasQa?.MASTER_CANVAS_QA === "NEED_SOURCE_VERIFICATION" && (
            <p className="text-xs text-amber-700">NEED_SOURCE_VERIFICATION: {masterCanvasQa.details[0]}</p>
          )}
          {showCameraPlan && masterCanvas && (
            <div className="border-t border-slate-100 pt-3 space-y-1">
              {masterCanvas.cameraShots.map((shot) => (
                <div key={shot.id} className="flex items-center justify-between text-[11px] text-slate-600">
                  <span className="font-mono">{shot.sequenceIndex}. {shot.type} → {shot.targetId}</span>
