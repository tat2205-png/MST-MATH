...
export function TeacherWorkspace({ onOpenStudio }: { onOpenStudio: () => void }) {
  ...
  const queryQuestions = useCallback(async (overrides: { statuses?: Array<"APPROVED" | "REVIEW" | "QUARANTINED"> } = {}) => {
    setBusy("query");
    try {
      const result = await api<QuestionQueryResponse>("/api/teacher-workflow/questions/query", { query: { text: searchText || undefined, types: type ? [type] : undefined, statuses: overrides.statuses ?? (status ? [status] : undefined), hasFigures: hasFigures === "" ? undefined : hasFigures === "yes", sort: { field: "INDEX", direction: "ASC" }, offset: 0, limit: 50 } });
      setQuestions(result.items); setTotal(result.total);
  ...
  }, [hasFigures, searchText, status, type]);
