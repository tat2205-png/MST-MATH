...
type ExamAnalysisView = { questionId: string; publicationStatus: string; qa: { status: string; issues: Array<{ code: string; message: string; severity: string }> }; proposals: Array<{ issueId: string; rationale: string; teacherDecision: string; verificationStatus: string }> };
type CorrectionDecision = "ACCEPT" | "EDIT" | "REJECT";
...
export function QuestionCard({ question, selected, onToggle, review, analysis, onAnalyze, onDecision }: { question: TeacherQuestion; selected?: boolean; onToggle?: () => void; review?: boolean; analysis?: ExamAnalysisView; onAnalyze?: () => void; onDecision?: (proposalId: string, decision: CorrectionDecision) => void; key?: React.Key }) {
    ...
    <article className={`rounded-xl border bg-white p-4 shadow-sm ${selected ? "border-blue-400 ring-2 ring-blue-100" : "border-slate-200"}`}>
      ...
      {review && (
        <div className="mt-3 border-t border-slate-100 pt-3 text-xs text-slate-600">
          ...
          {onAnalyze && <button onClick={onAnalyze} className="mt-2 rounded-lg border border-blue-200 bg-blue-50 px-3 py-1.5 font-semibold text-blue-700">Phân tích lỗi và đề xuất sửa</button>}
          {analysis && <div data-exam-analysis className="mt-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-amber-900"><strong>{analysis.publicationStatus}</strong><div className="mt-1">{analysis.qa.issues.length} vấn đề · {analysis.proposals.length} đề xuất</div>{analysis.proposals.map((proposal) => <div key={proposal.issueId} className="mt-2 border-t border-amber-200 pt-2"><span className="font-mono">{proposal.issueId}</span> · {proposal.teacherDecision} · {proposal.verificationStatus}<div>{proposal.rationale}</div>{proposal.teacherDecision === "PENDING" && onDecision && <div className="mt-2 flex flex-wrap gap-2"><button onClick={() => onDecision(proposal.issueId, "ACCEPT")} className="rounded border border-emerald-300 bg-emerald-50 px-2 py-1 font-semibold text-emerald-700">Accept</button><button onClick={() => onDecision(proposal.issueId, "EDIT")} className="rounded border border-blue-300 bg-blue-50 px-2 py-1 font-semibold text-blue-700">Edit</button><button onClick={() => onDecision(proposal.issueId, "REJECT")} className="rounded border border-rose-300 bg-rose-50 px-2 py-1 font-semibold text-rose-700">Reject</button></div>}</div>)}</div>}
...
export function TeacherWorkspace({ onOpenStudio }: { onOpenStudio: () => void }) {
  ...
  const [examAnalyses, setExamAnalyses] = useState<Record<string, ExamAnalysisView>>({});
  ...
  const analyzeQuestion = async (questionId: string) => {
    try { const result = await api<ExamAnalysisView>("/api/teacher-workflow/exam-analysis", { questionId }); setExamAnalyses((current) => ({ ...current, [questionId]: result })); }
    catch (error) { setNotice({ tone: "error", title: "Phân tích đề thất bại", message: error instanceof Error ? error.message : String(error) }); }
  };

  const decideCorrection = async (question: TeacherQuestion, proposalId: string, decision: CorrectionDecision) => {
    let revisedQuestion = question;
    if (decision === "EDIT") {
      const replacement = window.prompt("Nội dung thay thế cho đề bài:", question.stem.map((block) => block.type === "text" ? block.value : "").join(" "));
      if (replacement === null) return;
      revisedQuestion = { ...question, stem: [{ type: "text", value: replacement }] };
    }
    try {
      const result = await api<{ question: TeacherQuestion; analysis: ExamAnalysisView }>("/api/teacher-workflow/corrections/decision", { questionId: question.id, proposalId, decision, revisedQuestion });
      setExamAnalyses((current) => ({ ...current, [question.id]: result.analysis }));
      setQuestions((current) => current.map((item) => item.id === question.id ? result.question : item));
      setNotice({ tone: "success", title: "Đã lưu quyết định", message: `${decision}: ${result.analysis.publicationStatus}` });
    } catch (error) { setNotice({ tone: "error", title: "Không thể xử lý đề xuất", message: error instanceof Error ? error.message : String(error) }); }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
        ...
        {area === "review" && <section><div className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-xl font-bold">Duyệt câu hỏi</h1><p className="mt-1 text-sm text-slate-600">Giáo viên có thể đọc lỗi, evidence và đề xuất trước khi quyết định. Câu QUARANTINED, INVALID, BLOCKED hoặc NOT_TESTED luôn bị chặn.</p></div><button disabled={busy !== null} onClick={approveSelected} className="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700 disabled:opacity-50">Duyệt các câu hợp lệ {selectedIds.length ? `(${selectedIds.length})` : ""}</button></div>{importResult?.p01 && <div data-p01-review-state className={`mt-4 rounded-xl border p-4 text-sm ${importResult.p01.status === "PASS" ? "border-emerald-200 bg-emerald-50 text-emerald-900" : "border-amber-200 bg-amber-50 text-amber-900"}`}><strong>P01 source-backed state: {importResult.p01.status}</strong><div className="mt-1 font-mono text-xs">sourceHash={importResult.p01.sourceHash}</div>{importResult.p01.artifactFormats?.length ? <div className="mt-1">Output authority: {importResult.p01.artifactFormats.join(", ")}</div> : <div className="mt-1">Chưa xuất bản; cần hoàn tất teacher review.</div>}</div>}{importResult?.diagnostics.length ? <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><strong>Chẩn đoán nhập:</strong><ul className="mt-2 list-disc space-y-1 pl-5">{importResult.diagnostics.map((item, index) => <li key={`${item.code}-${index}`}>{item.code}{item.questionId ? ` · ${item.questionId}` : ""}</li>)}</ul></div> : null}<div className="mt-5 space-y-4">{questions.length ? questions.map((question) => <QuestionCard key={question.id} question={question} selected={selectedIds.includes(question.id)} onToggle={() => toggleSelected(question.id)} review analysis={examAnalyses[question.id]} onAnalyze={() => void analyzeQuestion(question.id)} onDecision={(proposalId, decision) => void decideCorrection(question, proposalId, decision)} />) : <Empty title="Không còn câu hỏi cần duyệt" action="Mở ngân hàng câu hỏi" onAction={() => navigate("bank")} />}</div></section>}
