...
import { readinessActionValues } from "../../services/teacherWorkflowTypes.js";
...
type AuthoritativeQaState = "PASS" | "WARN" | "REVIEW" | "FAIL" | "UNKNOWN";
type TeacherAuthoritativeReadiness = {
  source: { state: AuthoritativeQaState; reasons: string[] };
  process: { state: AuthoritativeQaState; reasons: string[] };
  design: { state: AuthoritativeQaState; artifactId: string | null; assessmentId: string | null; reasons: string[] };
  qa: { state: AuthoritativeQaState; artifactId: string | null; assessmentId: string | null; reasons: string[] };
  exportCapability: { available: boolean; supportedFormats: ExportFormat[] };
  currentArtifactExportReadiness: { ready: boolean; artifactId: string | null; assessmentId: string | null; allowedFormats: ExportFormat[]; reasons: string[] };
};
...
async function api<T>(path: string, body?: unknown): Promise<T> {
  ...
  return (data.result ?? data.summary ?? data.readiness) as T;
}
...
function StatusBadge({ status }: { status?: string }) {
  const tone = status === "APPROVED" || status === "VALID" || status === "READY" || status === "PASS" ? "bg-emerald-50 text-emerald-700 border-emerald-200" : status === "QUARANTINED" || status === "INVALID" || status === "BLOCKED" || status === "FAIL" ? "bg-rose-50 text-rose-700 border-rose-200" : "bg-amber-50 text-amber-800 border-amber-200";
...
export function TeacherWorkspace({ onOpenStudio }: { onOpenStudio: () => void }) {
  ...
  const [authoritativeReadiness, setAuthoritativeReadiness] = useState<TeacherAuthoritativeReadiness | null>(null);
  ...
  const loadAuthoritativeReadiness = useCallback(async (assessmentId?: string) => {
    try {
      const query = assessmentId ? `?assessmentId=${encodeURIComponent(assessmentId)}` : "";
      setAuthoritativeReadiness(await api<TeacherAuthoritativeReadiness>(`/api/teacher-workflow/readiness${query}`));
    } catch {
      setAuthoritativeReadiness(null);
    }
  }, []);
  ...
  useEffect(() => { void loadSummary(); void loadAuthoritativeReadiness(); }, [loadAuthoritativeReadiness, loadSummary]);
  ...
  const assessmentCapabilityReady = summary?.readiness.actions.assessment.ready === true;
  const gameCapabilityReady = summary?.readiness.actions.game.ready === true;
  const videoCapabilityReady = summary?.readiness.actions.video.ready === true;
  const exportCapabilityReady = summary?.readiness.actions.export.ready === true;
  const runtimeCapabilitiesReady = summary !== null && readinessActionValues(summary.readiness.actions).every((action) => action.ready);
  const currentAssessmentQaState = assessment && authoritativeReadiness?.qa.assessmentId === assessment.assessment.id ? authoritativeReadiness.qa.state : "UNKNOWN";
  const selectedFormatsAllowed = exportFormats.length > 0 && exportFormats.every((format) => authoritativeReadiness?.currentArtifactExportReadiness.allowedFormats.includes(format));
  const currentArtifactExportReady = Boolean(
    assessment &&
    authoritativeReadiness?.currentArtifactExportReadiness.ready &&
    authoritativeReadiness.currentArtifactExportReadiness.assessmentId === assessment.assessment.id &&
    selectedFormatsAllowed
  );
  ...
  const runImport = async () => {
    ...
    try {
      ...
      setImportResult(result); setSummary(result.summary); setQuestions(result.imported); setAuthoritativeReadiness(null); setArea("review");
  ...
  const approveSelected = async () => {
    ...
    setBusy("approve");
    try { setSummary(await api<WorkflowSummary>("/api/teacher-workflow/approve", { ids })); setSelectedIds([]); await queryQuestions({ statuses: ["REVIEW", "QUARANTINED"] }); await loadAuthoritativeReadiness(assessment?.assessment.id); setNotice({ tone: "success", title: "Đã duyệt", message: `${ids.length} câu hỏi hợp lệ đã được chuyển vào nhóm APPROVED.` }); }
    catch (error) { setNotice({ tone: "error", title: "Không thể duyệt", message: error instanceof Error ? error.message : String(error) }); }
    finally { setBusy(null); }
  };

  const generateAssessment = async () => {
    if (!assessmentCapabilityReady) return;
    ...
    try {
      ...
      setAssessment(result); await loadAuthoritativeReadiness(result.assessment.id); setArea("assessment"); setNotice({ tone: "success", title: "Đã tạo đề", message: `Đề gồm ${result.questionIds.length} tham chiếu Question ID; câu hỏi nguồn không bị sao chép hoặc sửa đổi.` });
  ...
  const startGame = async () => {
    if (!assessment || !gameCapabilityReady) return;
    setBusy("game");
  ...
  const prepareVideo = async () => {
    ...
    if (!questionId || !videoCapabilityReady) return;
  ...
  const runExport = async () => {
    if (!assessment || !exportCapabilityReady) return;
    setBusy("export");
    try {
      const readiness = await api<TeacherAuthoritativeReadiness>(`/api/teacher-workflow/readiness?assessmentId=${encodeURIComponent(assessment.assessment.id)}`);
      setAuthoritativeReadiness(readiness);
      const formatsAllowed = exportFormats.length > 0 && exportFormats.every((format) => readiness.currentArtifactExportReadiness.allowedFormats.includes(format));
      if (!readiness.currentArtifactExportReadiness.ready || !formatsAllowed) {
        throw new Error(`EXPORT_NOT_READY:${readiness.currentArtifactExportReadiness.reasons.join(",") || "QA_UNKNOWN"}`);
      }
      const result = await api<ExportWorkflowResult>("/api/teacher-workflow/exports", { assessmentId: assessment.assessment.id, audience: exportAudience, formats: exportFormats, includeAnswers: exportAudience === "TEACHER" && includeAnswers, includeSolutions: exportAudience === "TEACHER" && includeSolutions, filename: "de-kiem-tra" });
      setExportResult(result); setArea("export");
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <header className="sticky top-0 z-40 border-b border-slate-800 bg-slate-900 text-white shadow-md">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6">
          ...
          <div className="flex items-center gap-2 text-xs">
            <span className={`rounded-full border px-3 py-1.5 ${runtimeCapabilitiesReady ? "border-emerald-500/40 bg-emerald-500/10 text-emerald-300" : "border-amber-500/40 bg-amber-500/10 text-amber-200"}`}>{summary === null ? "Runtime chưa xác minh" : runtimeCapabilitiesReady ? "Runtime capability READY" : "Runtime cần kiểm tra"}</span>
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
        ...
        {area === "export" && <section className="grid gap-6 lg:grid-cols-[380px_1fr]"><div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><h1 className="text-xl font-bold">Xuất đề và tài liệu</h1><p className="mt-2 text-sm text-slate-600">Dùng QB-2D. Chế độ học sinh luôn chặn đáp án và lời giải.</p><div className="mt-3 rounded-lg bg-slate-50 p-3 text-xs text-slate-600">Authoritative export readiness: <strong>{currentArtifactExportReady ? "READY" : "BLOCKED/REVIEW"}</strong>{authoritativeReadiness?.currentArtifactExportReadiness.reasons.length ? ` · ${authoritativeReadiness.currentArtifactExportReadiness.reasons.join(", ")}` : ""}</div><FormSelect label="Đối tượng" value={exportAudience} onChange={(value) => { setExportAudience(value as ExportAudience); if (value === "STUDENT") { setIncludeAnswers(false); setIncludeSolutions(false); } }} options={[["STUDENT", "Học sinh"], ["TEACHER", "Giáo viên"]]} /><fieldset className="mt-4"><legend className="text-xs font-semibold">Định dạng</legend><div className="mt-2 grid grid-cols-2 gap-2">{(["JSON", "LATEX", "DOCX", "PDF"] as ExportFormat[]).map((format) => <label key={format} className="flex items-center gap-2 rounded-lg border border-slate-200 p-2 text-sm"><input type="checkbox" checked={exportFormats.includes(format)} onChange={() => setExportFormats((current) => current.includes(format) ? current.filter((item) => item !== format) : [...current, format])} />{format}</label>)}</div></fieldset><label className="mt-4 flex items-center gap-2 text-sm"><input type="checkbox" disabled={exportAudience === "STUDENT"} checked={includeAnswers} onChange={(e) => setIncludeAnswers(e.target.checked)} />Kèm đáp án</label><label className="mt-2 flex items-center gap-2 text-sm"><input type="checkbox" disabled={exportAudience === "STUDENT"} checked={includeSolutions} onChange={(e) => setIncludeSolutions(e.target.checked)} />Kèm lời giải</label>{exportAudience === "STUDENT" && <p className="mt-2 text-xs text-amber-700">Đáp án, lời giải và metadata giáo viên bị khóa trong bản học sinh.</p>}<button disabled={!assessment || !currentArtifactExportReady || !exportCapabilityReady || busy !== null} onClick={runExport} className="mt-5 w-full rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800 disabled:opacity-40">Tạo artifact thực tế</button></div><div>{!assessment ? <Empty title="Chưa có Assessment để xuất" action="Tạo đề" onAction={() => navigate("assessment")} /> : !exportResult ? <Empty title={currentArtifactExportReady ? "Chưa có artifact xuất bản" : "Artifact chưa đạt authoritative readiness"} action={currentArtifactExportReady ? "Chọn định dạng và xuất" : "Quay lại kiểm tra đề"} onAction={currentArtifactExportReady ? runExport : () => navigate("assessment")} /> : <div className="rounded-xl border border-emerald-200 bg-white p-5 shadow-sm"><StatusBadge status="READY" /><h2 className="mt-2 text-lg font-bold">Artifact đã tạo</h2><p className="mt-1 text-xs text-slate-500">{exportResult.audience} · {exportResult.questionIds.length} Question ID</p><div className="mt-4 space-y-3">{exportResult.artifacts.map((artifact) => <div key={artifact.format} className="rounded-lg border border-slate-200 p-3"><div className="flex items-center justify-between"><strong>{artifact.format}</strong><span className="text-xs text-slate-500">{artifact.bytes} bytes</span></div><p className="mt-1 break-all font-mono text-xs text-slate-500">{artifact.path}</p><span className="mt-2 inline-block text-xs font-semibold text-emerald-700">Đã xác minh tồn tại · SHA-256 {artifact.sha256.slice(0, 12)}…</span></div>)}</div></div>}</div></section>}
      </main>
