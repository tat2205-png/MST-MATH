...
function assertCurrentGameQuestion(question: QuestionObject | undefined, ref: GameQuestionRef): QuestionObject {
  if (!question) throw new Error(`GAME_QUESTION_NOT_FOUND:${ref.questionId}`);
  if (question.bankStatus !== "APPROVED") throw new Error(`GAME_QUESTION_NOT_APPROVED:${ref.questionId}`);
  if (
