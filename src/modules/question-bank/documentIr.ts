...
function parseParagraph(node: unknown, rels: Map<string, string>): ParagraphParse {
  ...
  const walk = (current: unknown) => entries(current).forEach(([key, value, holder]) => {
    if (key === "w:t") { const text = textValue(value); if (text.trim()) mergeContent(result.content, { type: "text", value: text }); return; }
  ...
  walk(node); result.content = result.content.filter((block) => block.type !== "text" || block.value.trim().length > 0); return result;
}
