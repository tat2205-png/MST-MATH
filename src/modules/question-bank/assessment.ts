...
import { QuestionSearchService } from "./search.js";
import type {
  ContentBlock,
...
export interface AssessmentSectionSpec {
  ...
  count: number;
  filters?: Omit<QuestionSearchQuery, "types" | "statuses" | "sort" | "limit" | "offset">;
...
export interface AssessmentSpec {
  ...
  globalFilters?: Omit<QuestionSearchQuery, "types" | "statuses" | "sort" | "limit" | "offset">;
...
export type AssessmentGenerationResult = AssessmentGenerationSuccess | AssessmentGenerationFailure;

const hash = (value: unknown) =>
  createHash("sha256").update(JSON.stringify(value)).digest("hex");
const seedNumber = (seed: string) => Number.parseInt(hash(seed).slice(0, 8), 16) >>> 0;
...
const mergeFilters = (
  ...
  ...globalFilters,
  ...localFilters,
  metadata: { ...globalFilters?.metadata, ...localFilters?.metadata },
});
...
const validStatuses = new Set<QuestionBankStatus>(["APPROVED", "REVIEW", "QUARANTINED"]);
...
export function validateAssessmentSpec(spec: AssessmentSpec): AssessmentDiagnostic[] {
  ...
  if (
    ...
    spec.statuses?.some((status) => !validStatuses.has(status) || status !== "APPROVED")
  ) {
    diagnostics.push({
      code: "INVALID_ASSESSMENT_SPEC",
      message: "A valid seed, APPROVED-only status policy, and at least one section are required.",
    });
  }
  ...
  for (const section of Array.isArray(spec.sections) ? spec.sections : []) {
    if (
      ...
      (section.pointsPerQuestion !== undefined &&
        (!Number.isFinite(section.pointsPerQuestion) || section.pointsPerQuestion < 0)) ||
      (section.ordering !== undefined && !validOrders.has(section.ordering)) ||
      (section.sourcePolicy !== undefined && !validSourcePolicies.has(section.sourcePolicy))
    ) {
      diagnostics.push({
        code: "INVALID_ASSESSMENT_SPEC",
        message: "Section identifiers, types, counts, points, ordering, and source policy must be valid.",
...
export function createAssessmentQueryPlan(spec: AssessmentSpec): AssessmentQueryPlan[] {
...
function selectSources(
  ...
  if (policy === "DISTINCT_SOURCE_REQUIRED" && distinct.length < count) return undefined;
  return [...distinct, ...repeated].slice(0, count);
}

function finalIssues(spec: AssessmentSpec, sections: AssessmentSection[]): AssessmentDiagnostic[] {
  ...
  if (
    ...
    issues.push({
      ...
      message: "Selected questions violated count, identity, type, status, or provenance invariants.",
    });
  }
  return issues;
}
  ...
  generate(spec: AssessmentSpec): AssessmentGenerationResult {
    ...
    for (let i = 0; i < queryPlan.length; i++) {
      ...
      const knownMetadata = new Set(
        this.repository.load().questions.flatMap((q) => Object.keys(q.metadata)),
      );
      ...
      const safe = this.search.approvedForReuse({
        ...plan.query,
        duplicateStates: undefined,
        limit: 100,
      });
      const possibleCount = safe.items.filter(
        (q) => q.duplicateState === "POSSIBLE_DUPLICATE",
      ).length;
      if (possibleCount) {
        diagnostics.push({
      ...
      let candidates = safe.items.filter(
        (q) =>
          q.duplicateState !== "DUPLICATE" &&
          q.duplicateState !== "POSSIBLE_DUPLICATE" &&
          !used.has(q.id),
      );
      ...
      sections.push({
        ...
        questionRefs: selected.map((q, index) => ({
          questionId: q.id,
          sectionId: sectionSpec.id,
          position: index + 1,
          type: q.type,
          status: q.bankStatus!,
          source: structuredClone(q.source),
    ...
    if (diagnostics.some((x) => x.code === "ASSESSMENT_FINAL_VALIDATION_FAILED")) {
      return { ok: false, diagnostics, queryPlan };
    }
    ...
    const entries = selectedIds
      .map((questionId) => this.search.getApprovedById(questionId)!)
      .filter((q) => q.answer?.length || q.solution?.length)
      .map((q) => ({
        questionId: q.id,
        answer: q.answer && structuredClone(q.answer),
        solution: q.solution && structuredClone(q.solution),
      }));
