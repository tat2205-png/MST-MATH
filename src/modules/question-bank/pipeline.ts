...
function canonicalizeCompositeAnchors(candidate: DocumentQuestionCandidate, document: DocumentIR): DocumentQuestionCandidate {
  const composites = candidate.figureAnchors.map((id) => document.figures.find((f) => f.id === id)).filter((f) => f?.componentIds?.length);
  const discarded = new Set<string>();
  for (const left of composites) for (const right of composites) if (left !== right && left!.componentIds!.some((id) => right!.componentIds!.includes(id))) {
