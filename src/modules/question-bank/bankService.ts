...
import type { DuplicateResult, FigureRecord, QuestionBankRepository, QuestionBankSnapshot, QuestionObject } from "./types.js";
...
export class QuestionBankService {
  ...
  private importPipeline(pipeline: ReturnType<typeof ingestDocxQuestions>): BankImportResult {
    ...
    const relations = before.relations ?? { schemaVersion: 1 as const, relations: [], families: [], duplicateAudit: [] };
    for (const source of pipeline.questions) { const question = statusQuestion(source); const duplicate = detectDuplicate(question, questions); const match = duplicate.matchedId ? questions.find(q => q.id === duplicate.matchedId) : undefined; const relation = match ? analyzeRelation(match, question) : undefined; if (relation) relations.relations.push(relation); duplicates.push(duplicate); if (relation?.relations.includes("EXACT_DUPLICATE")) { const kept = match ?? questions[0]; if (kept) relations.duplicateAudit.push({ removedQuestionId: question.id, keptQuestionId: kept.id, relation: "EXACT_DUPLICATE", sourceDocument: question.source.document, sourceHash: question.source.sourceHash, sourceLocations: question.source.sourceLocations, evidence: relation?.evidence ?? duplicate.evidence, policyVersion: "PIMATH_QUESTION_RELATIONS_V1" }); diagnostics.push({ code: "DUPLICATE_QUESTION", questionId: question.id, severity: "INFO", details: { matchedId: kept?.id } }); continue; } question.duplicateState = relation?.relations.includes("POSSIBLE_DUPLICATE") ? "POSSIBLE_DUPLICATE" : duplicate.status; if (relation?.relations.includes("SOURCE_CONFLICT")) diagnostics.push({ code: "SOURCE_CONFLICT", questionId: question.id, severity: "WARNING", details: { matchedId: match?.id } }); question.figures.forEach((figure) => registry.register(figure)); if (question.bankStatus === "QUARANTINED") diagnostics.push({ code: "INVALID_QUESTION_STRUCTURE", questionId: question.id, severity: "ERROR" }); questions.push(question); imported.push(question); if (relation?.relations.includes("PARAMETRIC_VARIANT") && match) { const familyId = familyIdFor(match, question); const family = relations.families.find(f => f.familyId === familyId) ?? { familyId, memberQuestionIds: [match.id], relationEvidence: relation.evidence, schemaVersion: 1 as const }; if (!family.memberQuestionIds.includes(question.id)) family.memberQuestionIds.push(question.id); if (!relations.families.includes(family)) relations.families.push(family); } }
