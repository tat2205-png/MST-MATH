...
export function foldNetToSvg(scene:FoldScene):FoldRendererBoundaryResult&{output?:string}{
  ...
  const stroke=getNAMathVisualStyle("visible-edge").color;const polygons=scene.net.faces.map(face=>`<polygon id="${escapeXml(face.faceId)}" data-source-face="${escapeXml(face.sourceFaceId)}" points="${face.vertices.map(v=>`${n(sx(v.position[0]))},${n(sy(v.position[1]))}`).join(" ")}" fill="none" stroke="${stroke}" stroke-width="2"/>`).join("");return{status:"PASS",renderer:"svg",available:true,scene,issues:[],output:`<svg xmlns="http://www.w3.org/2000/svg" width="640" height="480" viewBox="0 0 640 480">${polygons}</svg>`};
}
