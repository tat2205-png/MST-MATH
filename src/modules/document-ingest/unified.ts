...
function figureFromAsset(asset: ExtractedDocxAsset, sourceLocation: string): FigureRecord {
  const mime = mediaType(asset);
  const vector = /(?:wmf|emf)/i.test(mime) || /\.(?:wmf|emf)$/i.test(asset.filename);
  return {
    id: asset.id,
    ...
    semanticRole: vector ? "REAL_FIGURE" : "RASTER_FIGURE",
    derivation: {
...
function mapDocxBlock(
  ...
  if (block.type === "paragraph") {
    const content = block.children.flatMap((child) => mapInline(child, assetById, oleLatex, figures, diagnostics));
  ...
  const cells = block.rows.flatMap((row) => row.cells.map((cell) => cell.blocks.flatMap((cellBlock) => cellBlock.type === "paragraph" ? cellBlock.children.flatMap((child) => mapInline(child, assetById, oleLatex, figures, diagnostics)) : [])));
  return {
    id: `word-table-${order}`,
    kind: "TABLE",
    order,
...
function ingestWord(source: UnifiedInputSource): UnifiedInputResult {
  ...
  const blocks = parsed.ast.blocks.map((block, index) => mapDocxBlock(block, index, assetById, oleLatex, figures, diagnostics)).filter((block): block is DocumentBlock => Boolean(block));
  ...
  return { ok: status !== "FAIL", status, kind: "WORD", document, diagnostics, classification: classify(document), provider: legacyIds.size ? "OMML+MTEF" : "OMML" };
}
...
async function ingestRecognized(source: UnifiedInputSource, kind: "PDF" | "IMAGE", options: UnifiedInputOptions): Promise<UnifiedInputResult> {
  ...
  try {
    ...
    const status: UnifiedInputStatus = diagnostics.some((d) => d.severity === "error") ? "FAIL" : diagnostics.length ? "REVIEW_REQUIRED" : "PASS";
    return { ok: status !== "FAIL", status, kind, document: { ...document, warnings: diagnostics.map((d) => d.code) }, diagnostics, classification: classes, provider: output.provider };
  } catch (error) {
...
export async function ingestUnifiedSource(source: UnifiedInputSource, options: UnifiedInputOptions = {}): Promise<UnifiedInputResult> {
  ...
  if (kind === "WORD") return ingestWord(source);
