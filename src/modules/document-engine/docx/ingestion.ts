...
export function ingestDocx(source: DocxIngestionSource): DocxIngestionResult {
  ...
  const assetKind = (asset: FigureRecord): AssetKind => /\.wmf$/i.test(asset.mediaPath ?? "") ? "WMF" : /\.emf$/i.test(asset.mediaPath ?? "") ? "EMF" : asset.layout ? "VML" : "IMAGE";
  for (const figure of figures) if (!assets.some(asset => asset.assetId === figure.id)) assets.push({ assetId: figure.id, kind: assetKind(figure), relationshipId: figure.relationshipId, sourceAnchor: figure.sourceAnchor ?? anchor(sourceDocumentId, figure.mediaPath ?? "word/document.xml", 0, undefined, undefined, figure.relationshipId), originalFilename: figure.mediaPath?.split("/").at(-1), contentType: figure.mimeType, sha256: figure.bytes ? hashBytes(figure.bytes) : undefined, status: figure.status ?? "PASS" });
