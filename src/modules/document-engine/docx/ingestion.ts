...
import type { AssetKind, ContentBlock, DocumentBlock, DocumentIR, ExtractionIssue, FigureRecord, MathLedgerEntry, PreservationStatus, Provenance, SourceAnchor } from "../document-ir.js";
...
export function ingestDocx(source: DocxIngestionSource): DocxIngestionResult {
  ...
  const blocks: DocumentBlock[] = parsed.ast.blocks.map((block, index) => {
    ...
    if (block.type === "table") { assets.push({ assetId: `table-${block.index}`, kind: "TABLE", sourceAnchor: anchor(sourceDocumentId, "word/document.xml", block.index, undefined, block.index), status: "PASS" }); return { id: `table-${block.index}`, kind: "TABLE", order: index, content: [{ type: "table", cells: tableContent(block, sourceDocumentId, math, assets, issues), sourceLocation: `word/document.xml/body/tbl[${block.index}]` }], sourceLocation: `word/document.xml/body/tbl[${block.index}]` }; }
  ...
  const assetKind = (asset: FigureRecord): AssetKind => /\.wmf$/i.test(asset.mediaPath ?? "") ? "WMF" : /\.emf$/i.test(asset.mediaPath ?? "") ? "EMF" : asset.semanticRole === "VML" ? "VML" : asset.semanticRole === "DRAWING" ? "DRAWINGML" : "IMAGE";
  for (const figure of figures) assets.push({ assetId: figure.id, kind: assetKind(figure), relationshipId: figure.relationshipId, sourceAnchor: figure.sourceAnchor ?? anchor(sourceDocumentId, figure.mediaPath ?? "word/document.xml", 0, undefined, undefined, figure.relationshipId), originalFilename: figure.mediaPath?.split("/").at(-1), contentType: figure.mimeType, sha256: figure.bytes ? hashBytes(figure.bytes) : undefined, status: figure.status ?? "PASS" });
